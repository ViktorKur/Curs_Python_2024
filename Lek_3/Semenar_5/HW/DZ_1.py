...
# Мой вариант решения (подсмотрен)
...
site = {'html': {'head': {'title': 'Мой сайт'},'body': {'h2': 'Здесь будет мой заголовок','div': 'Тут, наверное, какой-то блок', 'p': 'А вот здесь новый абзац'}}}
iskom_key = input('Введите искомый ключ: ') 
vibor = input('Хотите ввести максимальную глубину? Y/N: ').upper() 
if vibor == 'Y': 
    depth = int(input('Введите максимальную глубину: ')) 
    print(f'Значение ключа: {search_key(iskom_key, site, depth)}') 
elif vibor == 'N': 
    print(f'Значение ключа: {search_key(iskom_key , site)}')
else:
    print("Неправильный параметр, нужно было ввести Y или N, попробуйте еще раз") 
